...
type FlowRecord struct {
	...
	FlowId        int64  `json:"flow_id"         ` // Unique identifier for the flow
	DeviceId      string `json:"device_id"       ` // ID of the device
	StartTime     int64  `json:"start_time"      ` // Start time of the flow in epoch milliseconds
	EndTime       int64  `json:"end_time"        ` // End time of the flow in epoch milliseconds
	...
	PcktToSrv     int    `json:"pckt_to_srv"     ` // Packets sent to the server
	PcktToClient  int    `json:"pckt_to_client"  ` // Packets sent to the client
	BytesToSrv    int    `json:"bytes_to_srv"    ` // Bytes sent to the server
	BytesToClient int    `json:"bytes_to_client" ` // Bytes sent to the client
	Proto         string `json:"protocol"        ` // Protocol used (e.g., TCP, UDP)
	Pcap          string `json:"pcap"            ` // Associated PCAP file name
